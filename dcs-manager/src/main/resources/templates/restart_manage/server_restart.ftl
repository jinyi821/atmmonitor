...
<script>
    $(document).ready(function(){
        /*  $('#serverStop').click(function() {
             ...
             $('#serverStart').bind('click', function() {
                 serverStartProcess();
           });
           $('#serverRestart').bind('click', function() {
               serverRestartProcess();
           });*/
    });
    //dom操作
    var domFun={   };
    function  serverStopProcess(){
        /*bootsrap禁用用按钮*/
        $("#serverStop").addClass("disabled");
        $("#serverStop").prop("disabled", true);
        $("#serverStop").siblings().prop("disabled", true);
        $("#serverStop").button('loading');
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var html = [];
                html.push('<div class="row">server停止执行命令及响应信息</div>');
                var shellUsesArray=[];
                ...
                /*bootsrap启用按钮*/
                $("#serverStop").removeClass("disabled");
                $("#serverStop").prop("disabled",false);
                $("#serverStop").siblings().prop("disabled", false);
                $("#serverStop").button('reset');
    ...
    function  serverStartProcess(){
        $("#serverStart").addClass("disabled");
        $("#serverStart").prop("disabled", true);
        $("#serverStart").siblings().prop("disabled", false);
        $("#serverStart").button('loading');
        ...
        $.ajax({
            ...
            success: function (data) {
                var shellExecList=eval(data);
                var html = [];
                html.push('<div class="row">server启动执行命令及响应信息</div>');
                var shellUsesArray=[];
                var shellSubUsesArray=[];
                for(var i=0;i<shellExecList.length;i++){
                    var shellExecBody=shellExecList[i];
                ...
                $("#mainTab .tablebody").html(html.join(''));
                $("#serverStart").removeClass("disabled");
                $("#serverStart").prop("disabled",false);
                $("#serverStart").siblings().prop("disabled", false);
                $("#serverStart").button('reset')

            }
        })
    };

    function  serverRestartProcess(){
        $("#serverRstart").addClass("disabled");
        $("#serverRstart").prop("disabled",true);
        $("#serverRstart").siblings().prop("disabled", true);
        $("#serverRstart").button('loading');

        $("#mainTab .tablebody").empty();
        $.ajax({
            type: "get",
            url: "${BASE_PATH}/restartManage/serverRestart/serverRestartProcess",    //向后端请求数据的url
            cache:false,  //get方式相同参数请求，易产生缓存；用此参数设置清除缓存
            async:true,
            data: {},
            success: function (data) {
                var shellExecList=eval(data);
                var html = [];
                html.push('<div class="row">server重启动执行命令及响应信息</div>');
                var shellUsesArray=[];
                var shellSubUsesArray=[];
                for(var i=0;i<shellExecList.length;i++){
                    var shellExecBody=shellExecList[i];
                ...
                $("#mainTab .tablebody").html(html.join(''));
                $("#serverRstart").removeClass("disabled");
                $("#serverRstart").prop("disabled",false);
                $("#serverRstart").siblings().prop("disabled", false);
                $("#serverRstart").button('reset');
            }
        })
    };
...
<div id="page-inner" style="margin-top:-50px;">
    ...
    <div class="row" style="margin-top:15px;">
        <div class="toolBar">
            &nbsp;&nbsp;<div id="serverStop" title="" onClick="serverStopProcess();"  class="btn btn-success" data-loading-text="停止进行中..." data-original-title="">停止</div>
            &nbsp;&nbsp;&nbsp;&nbsp; <div id="serverStart" onClick="serverStartProcess();"  title="" class="btn btn-success" data-loading-text="启动进行中..." data-original-title="">启动</div>
            &nbsp;&nbsp;&nbsp;&nbsp;<div id="serverRstart" onClick="serverRestartProcess();"  title="" class="btn btn-success" data-loading-text="重启进行中..." data-original-title="">重启</div>
        </div>
    </div>
