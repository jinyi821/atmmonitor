...
<script>
    var serverStopValidateFlag="false";
    var serverStopProcessFlag="false";
    var serverStartValidateFlag="false";
    var serverStartProcessFlag="false";
    ...
    var eventFun={
        setStep:function(index){
            if(index==2 && serverStopValidateFlag=="true"){
                changeStepAndContent(index);
            }
            if(index==3 && serverStopProcessFlag=="true"&&serverStartValidateFlag=="true"){
                changeStepAndContent(index);
            }
            if(index==4 && serverStartProcessFlag=="true"){
                changeStepAndContent(index);
    ...
    function serverStopValidate(){
        $.ajax({
            type: "get",
            url: "${BASE_PATH}/restartManage/serverRestart/serverStopPreProcess",    //向后端请求数据的url
            async:false,
            data: {},
            success: function (data) {
                serverStopValidateFlag=data;
                if(serverStopValidateFlag=="true"){
                    /*  var html = [];
                    ...
                    //div.append(divchild);
                    var html = [];
                    html.push('<div class="row">server停止前环境条件已具备，可进入下一步server停止操作！</div>');
                    $("#step1 .tablebody").append(html.join(''));
                }else{
                    var html = [];
                    html.push('<div class="row">server停止前环境条件尚不具备，请检查停止server相关scan、probe等server是否已停止！</div>');
                    $("#step1 .tablebody").append(html.join(''));
                }
            }
        })
    };
    function  serverStopProcess(){
        $.ajax({
            type: "get",
            url: "${BASE_PATH}/restartManage/serverRestart/serverStopProcess",    //向后端请求数据的url
            async:true,
            data: {},
            success: function (data) {
                ...
                serverStopProcessFlag="true";
            }
        })
    };

    function serverStartValidate(){
        $.ajax({
            type: "get",
            url: "${BASE_PATH}/restartManage/serverRestart/serverStartPreProcess",    //向后端请求数据的url
            async:true,
            data: {},
            success: function (data) {
                serverStartValidateFlag=data;
                var html = [];
                if(serverStartValidateFlag=="true"){
                    html.push('<div class="row">server启动前环境条件已具备，可进入下一步server启动操作！</div>');
                    eventFun.setStep(3);
                }else{
                    var html = [];
                    html.push('<div class="row">server启动前环境条件尚不具备，请检查停止server相关scan、probe等server是否已启动！</div>');
                }
                $("#step2 .tablebody").append(html.join(''));
            }
        })
    };
    function  serverStartProcess(){
        $.ajax({
            type: "get",
            url: "${BASE_PATH}/restartManage/serverRestart/serverStartProcess",    //向后端请求数据的url
            async:true,
            data: {},
            success: function (data) {
                var shellExecList=eval(data);
    ...
    <div class="row" style="margin-top:0px">
        ...
        <div id="myTabContent" class="tab-content">
            <div id="step1" class="tab-pane fade active in">
                <div class="toolBar">
                    <a class="btn btn_primary" id="nextBtn"  style="float:left;" href="javascript:void(0);"  onclick="javascript:serverStopValidate();" >停止前验证</a>
                    <a class="btn btn_primary mt20" style="float:right;margin-right: 10px;" href="javascript:eventFun.setStep(2);">进入下一步</a>
                </div>
                <div class="tablebody">
                    <div class="row">server重启流程分停止、启动及相关校验验证操作，请按步骤流程操作！</div>
                </div>
            </div>
            <div id="step2" class="tab-pane fade">
                <div class="toolBar">
                    <a class="btn btn_primary" id="nextBtn"  style="float:left;" href="javascript:void(0);"  onclick="javascript:serverStopProcess();" >停止</a>
                    <a class="btn btn_primary mt20" style="float:right;margin-right: 10px;" href="javascript:serverStartValidate();">进入下一步</a>
                </div>
                <div class="tablebody">
                <#--<div class="row">server停止前环境条件已具备，可进入下一步server停止操作！</div>-->
                </div>
            </div>
            <div id="step3" class="tab-pane fade">
                <div class="toolBar">
                    <a class="btn btn_primary" id="nextBtn"  style="float:left;" href="javascript:void(0);"  onclick="javascript:serverStartProcess();" >启动</a>
            ...
            <div id="step4" class="tab-pane fade">
                <div class="apply-finish">
                    <div class="apply-finish-header">
                        ...
                        <div class="apply-finish-msg">恭喜您，已完成本次server重启流程操作！</div>
                    </div>
                </div>
            </div>
