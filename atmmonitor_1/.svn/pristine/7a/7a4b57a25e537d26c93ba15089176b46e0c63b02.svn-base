...
<body>
 <div class="divsection">             
	...
	<form action="${ctx}/equipmentchange/saveEquipmentChange.action" method="post"   name="myform" id="myform" target="_parent">
         ...
         <table class="base_table fontSize3 ContentTable">
          <tbody class="table_body">
			<tr>
				...
				<td width="35%">
				 <select id="equipmentid" name="equipmentid" class="js-example-basic-hide-search" style="width:90%;"  >
							<c:forEach items="${equipmentList}"  var="belong">
								<option  value="${belong.id}"
								<c:if test="${equipmentChange.equipmentid==belong.id}">selected</c:if>
								
								>${belong.depname}-${belong.name}</option>
							</c:forEach>
				 </select> 
				 <validation id="equipmentidV" dataType="Require" msg="设备名称必须填写！" /> 
               </td>
				<td width="10%" class="text_alignRight bgColor4">
				  <label class="fontSize3">审批人</label></td>
				<td width="40%" style="border-right:none;">
                      
				       <select id="approver" name="approver" class="js-example-basic-hide-search" style="width:90%;"  >
							<c:forEach items="${userList}"  var="belong">
							     
								<option  value="${belong.loginname}"
								<c:if test="${equipmentChange.approver==belong.loginname}">selected</c:if>
					 ...
					 <validation id="approverV" dataType="Require" msg="审批 人必须填写！" /> 
               </td>
			</tr>
			...
			<tr>
				...
				<td colspan="3">
			        <textarea id="applyremark" name="applyremark"  class="textAreaSy" style="width:95%;" placeholder="建议不要超过100字">${equipmentChange.applyremark}</textarea>
			    </td>			     
		   </tr>
		...
		<div class="base3">
		    <c:if test="${equipmentChange.status ==0}">
