                		...
                		<div class="wh_content">
                			<div class="rightDiv" style="margin: 0 auto; width:100%;">	                			
		        				 <div class="mxContent DivHeight marginBottom marginTop">
		                        	...
		                        	<input type="button" value="新增维修单" class="blueBtn" id="addrole" onclick="add()"  /> 
								<div class="searchInfo">
	                				...
	                				<c:set var="roleid" value="0"/>
		                                ...
		                                <td>@@{equipmentname}</td>
		                                <td>@@{applydept}</td>
		                                <td>@@{status[ENUM 1=待审批;2=审批通过;3=审批不通过;4=已维修;5=不能维修;0=新建]}</td>
		                                <td>@@{creater}</td>
...
　　                                                                                        <!--</c:if>-->
  ...
  <script type="text/javascript">
	...
	function preFunc(paraStr)
	{
		...
		paraStr.creater_condition=loginname;
	...
	var table = new ListTable("listtable", "${ctx}/commonQuery/query.action",10, [["sqlname", "SQL_Equipment_CHANGE.list"]],preFunc,dataFunc,tableBoot,parentiFrameHeight);
	function query()
	{
		table.submit();
	}
	function add()
	{
		...
		//window.open("${ctx}/usermanager/bsrole/bsRoleLoad.action");
		 layer.open({           
	            ...
	            title: ['新增维修单', 'font-size:14px;color:#333333;text-align:left;'],//弹窗标题的内容及对应样式
	            area: ['900px', '400px'], 
	            content: "${ctx}/equipmentchange/equipmentChangeDetail.action"
	...
	function openBsRole(roleid,status)
	{ 
		...
		layer.open({           
            type: 2,//设定弹窗为iframe嵌套页面
            title: ['编辑维修单', 'font-size:14px;color:#333333;text-align:left;'],//弹窗标题的内容及对应样式
            area: ['900px', '400px'], 
            content: '${ctx}/equipmentchange/equipmentChangeDetail.action?pid='+roleid+'&status='+status
           //content: ['baseStylePopModal.html', 'no'] //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
        });
	}
