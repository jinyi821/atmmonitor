...
<body>
 <div class="divsection">             
	...
	<form action="${ctx}/equipment/saveEquipment.action" method="post"   name="myform" id="myform" target="rightFrame2">
        <input type="hidden" name="id" id="id" value="${equipment.id}" />
        <input type="hidden" name="createtime" id="createtime" value="${equipment.createtime}" />
         ...
         <table class="base_table fontSize3 ContentTable">
          <tbody class="table_body">
			<tr>
				...
				<label class="fontSize3">ATM名称</label></td>				
				<td width="35%">
				 <input id="name" name="name" class="inputSy" type="text" value="${equipment.name}" /> 
				 <validation id="nameV" dataType="Require" msg="名称必须填写！" /> 
               </td>
				<td width="10%" class="text_alignRight bgColor4">
				  <label class="fontSize3">型号</label></td>
				<td width="40%" style="border-right:none;">

					 
					 <input type="text" id="type" class="inputSy" name="type" value="${equipment.type}"/>
					 <validation id="typeV" dataType="Require" msg="型号必须填写！" /> 
               </td>
			</tr>
			<tr>
					<td  class="text_alignRight bgColor4">
					     <label class="fontSize3">厂家</label></td>
					<td >
						<input type="text" id="factory" name="factory" class="inputSy" value="${equipment.factory}"/>	
						<validation id="factoryV" dataType="Require" msg="厂家必须填写！" />	
					</td>
					<td  class="text_alignRight bgColor4">
					     <label class="fontSize3">所属部门</label></td>
					<td >
						<select id="owndept" name="owndept" class="js-example-basic-hide-search" style="width:90%;">
							<c:forEach items="${deptList}"  var="belong">
								<option  value="${belong.pid}"
								<c:if test="${equipment.owndept==belong.pid}">selected</c:if>
								>${belong.depname}</option>
							</c:forEach>
						</select>
						<validation id="owndept" dataType="Require" msg="所属部门必须填写！" />	
					</td>
			</tr>
			<tr>
				...
				<td colspan="3">
			        <textarea id="remark" name="remark"  class="textAreaSy" style="width:95%;" placeholder="建议不要超过100字">${equipment.remark}</textarea>
			    </td>			     
		   </tr>
	...
	$(function(){
		var  productid="${equipment.id}" ;	
		if(productid!=null&&productid!=""){
			$("#isupdate").val(1);
			$("#functiontitle").text("编辑设备"); 		
		}else{
			$("#functiontitle").text("新增设备"); 	
		}
