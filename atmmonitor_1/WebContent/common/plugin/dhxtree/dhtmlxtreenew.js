...
function dhtmlXTreeObject(htmlObject, width, height, rootId){
  if (dhtmlxEvent.initTouch)
	...
	this.parentObject.style.overflow="hidden"; //不显示滚动条
   	this._itim_dg=true;
    this.dlmtr=",";
...
dhtmlXTreeObject.prototype._correctPlus=function(itemObject){
	if (!itemObject.htmlNode) return;
        ...
        var imsrc2=itemObject.htmlNode.childNodes[0].childNodes[0].childNodes[2].childNodes[0]; //div 对象
        var imsrc3=itemObject.htmlNode.childNodes[0].childNodes[0].childNodes[3].childNodes[1]; 
       // var imgsrc2td=itemObject.htmlNode.childNodes[0].childNodes[0].childNodes[2]; //td 对象
        
        var workArray=this.lineArray;
        if ((this.XMLsource)&&(!itemObject.XMLload)) {
        	var workArray=this.plusArray;
        	this._setSrc(imsrc2,this.iconURL+itemObject.images[2]);
        	this._setSrc(imsrc3,this.iconURL+itemObject.images[2]);
        	//imsrc2.style.display="none";
        	//imgsrc2td.style.display="none";
        	...
        	if ((itemObject.htmlNode.childNodes[0].childNodes[1])&&( itemObject.htmlNode.childNodes[0].childNodes[1].style.display!="none" )) {
        		...
        		this._setSrc(imsrc3,this.iconURL+itemObject.images[1]);
        		//imsrc2.style.display="none";
        		//imgsrc2td.style.display="none";
        		if (this._txtimg) return (imsrc.innerHTML="[-]");
        	} else {
        	...
        	this._setSrc(imsrc3,this.iconURL+itemObject.images[0]);
        	//imsrc2.style.display="none";
        	//imgsrc2td.style.display="none";
        }
...
dhtmlXTreeObject.prototype._createItem=function(acheck,itemObject,mode){
	...
	var td1=document.createElement('td');//树连接线所在位置
	...
	var td11=document.createElement('td'); //复选框所在位置
	...
	var td12=document.createElement('td');//imageArray leaf.gif、folderOpen.gif、folderClosed.gif 所放位置
	...
	td12.style.display="none"; 
        ...
        var td2=document.createElement('td'); //node中text值
        ...
        var img2=this._getImg(this.timgen?itemObject.id:this.rootId);
        img2.border="0";
        img2.style.display="inline-block";
    	if (this._aimgs){
      		img2.parentObject=itemObject;
      		if (img2.tagName=="IMG") img2.align="absmiddle";      	
      	} 
    	if (!mode) this._setSrc(img2,this.iconURL+this.imageArray[0]);
      	img2.style.padding=0; img2.style.margin=0;
      	if (this.timgen)
      	{  
      	img2.style.width=this.def_img_x; img2.style.height=this.def_img_y; 
      	}
      	else
      	{
      	img2.style.width="0px"; img2.style.height="0px";
        } 
