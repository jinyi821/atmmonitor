package com.common.core.component.data;
...
import com.common.core.component.database.ConnectionPool;
...
public class QueryAdapter {
	...
	private String dbType=PropertiesUtils.getProperty("jdbc.alias").toUpperCase();
	/**
...
//		int result=0;
	...
	public QueryAdapter(String poolName,String dbType) 
	{
		...
		dbType=StringUtils.checkNullString(dbType).trim();
		if(!"".equals(dbType))
		{
			this.dbType=dbType;
		}
...
//					pstmt.setObject( i, String.valueOf(p_pageSize));
	...
	public String executeQueryForJson(final String p_sql, final Object[] values,final int p_curpage, final int p_pageSize, final int p_isCount) 
	{
			...
			while(resultSet.next())
			{
				...
				for (i = 1; i <= cols; i++) 
				{
					...
					if(i>1)
						json.append(",");
					json.append("\"");
					json.append(colName.toLowerCase());
					json.append("\":");
					json.append(StringUtils.string2Json(StringUtils.checkNullString(value)));
				}
				//dataTable.putDataRow(dtRow);
				json.append("}");
			}//while(resultSet.next())
